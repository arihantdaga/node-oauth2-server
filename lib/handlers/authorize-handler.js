...
AuthorizeHandler.prototype.handle = function(request, response) {
  ...
  var fns = [
    ...
    this.getUserCustom(request,response)
  ];
...
AuthorizeHandler.prototype.getUserCustom = function(request,response){
  // return promisify(this.model.getUserCustom, 1).call(this.model, request).then(function(user){
  //   if (!user) {
  //     throw new ServerError('Server error: `handle()` did not return a `user` object');
  //   }
  //   return user;
  ...
  return new Promise((resolve,reject)=>{
    this.model.getUserCustom.call(this.model,request).then(function(user){
      return resolve(user);
    }).catch(function(err){
      return reject(err);
    })
...
AuthorizeHandler.prototype.updateResponse = function(response, redirectUri, state) {
  ...
  let redirect_url = url.format(redirectUri);
